...
def main():
    # Read data
    df = pd.read_csv(r'D:\Dev\Python\footballElo\processed_databases\transfermarkt\cleaned_games.csv')

    # Initialize clubs and ratings
    clubs = set(df['home_club_name']).union(set(df['away_club_name']))
    ...
    for idx, row in enumerate(df.itertuples(index=False), 1):
        home, away = row.home_club_name, row.away_club_name
        if home not in filtered_clubs or away not in filtered_clubs:
            continue

        home_goals, away_goals = row.home_club_goals, row.away_club_goals
        date = row.date
    ...
    elo_df.to_csv(f'elo_arpad_static_transfermarkt_{k}.csv')
